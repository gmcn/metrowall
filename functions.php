...
function starting_theme_scripts() {
	...
	wp_enqueue_script( 'jquery-3.5.1', 'https://code.jquery.com/jquery-3.5.1.min.js', array(), '3.5.1' );
...
add_image_size( 'download-pagination', 270, 355, true );
add_image_size( 'project-featured_small', 480, 480, true );
add_image_size( 'project-featured_large', 800, 800, true );
...
// Tag tracking brochure download
function metro_brochure_download_tracking() {
  if (is_page ('thank-you-brochure-download')) {
    ?>
        <!-- Event snippet for Brochure Download - Thank You conversion page -->
<script>
  gtag('event', 'conversion', {'send_to': 'AW-385898292/nZkCCNPKsowCELSugbgB'});
</script>

    <?php
  }
}
add_action('wp_head', 'metro_brochure_download_tracking');


// Tag tracking contact form
function metro_contact_form_tracking() {
...
add_action('wp_head', 'metro_contact_form_tracking');
